 ...
 *
 */
public class LiblinearFileGeneratorWithUndersampling {
	...
	public static void main(String[] args) throws Exception {
		...
		while ((currentLine = br.readLine()) != null) {
			if (count > 0) {
				row = "";
				index = 0; //1 cand am trasatura nr-of-words-in-concept
				...
				if (haveFilteredPOS(tokens, hs2POS, hs3POS, hs4POS, hs5POS, hs5CHNK)) {
					for (int i = 0; i < tokens.length; i++) {
						if (i == 0) { // lungime cuvant
							...
							Integer val = Integer.parseInt(tokens[i]);
							if (val != 0) { // && tokens[21].split("_").length == 1  // WN definition just for One-word concepts
								row += (index + 1) + ":" + val + " ";
							}
							index++;
						} else if (i > 0 && i <= 20) {
